...
Figure 1: The expected probabilities $P({S_{j,a}})$ of observing a segregating site at a locus for loci with different minor allele frequencies. Probabilities are derived from Equation 4 for each locus *a* for population sizes of 100 (left) and 1000 (right), each rarefacted to $n = 30$. Each locus has a random, independent percentage of missing data which varies between 0 and 30. Points are colored depending on if they fell within a 95% confidence level based on 10,000 random re-samplings of each population to 30 individuals.

Figure 2: The expected total number of segregating sites $E(N_{S})$ from populations of either 100 or 1,000 sampled to 30 individuals as calculated via Equation 5 vs that observed from 10,000 random re-samplings to 30 individuals. 95% Confidence and prediction intervals for $E(N_{S})$ are shown, calculated according to Equations 8 and 9. Observed $N_{S}$ values from each re-sample are colored depending on if they fell within the estimated 95% prediction interval. The mean $N_{S}$ across all re-samples is marked with a triangle, colored depending on if it fell within the estimated 95% confidence interval.