...
p1 <- ggplot(sub_res, aes(x = fis, y = value, group = fis)) +
  ...
  geom_point(data = pd[as.numeric(variable) != 4,],
...
# Figure S7
```{r}
S7 <- ggplot(sub_res, aes(x = fis, y = seg_sites, group = fis)) +
  geom_violin(draw_quantiles = c(.25, .5, .75)) +
...
ggsave("FigureS7.svg", S6, height = 10, width = 15, dpi = 1000) # doesn't want to save as pdf with break
